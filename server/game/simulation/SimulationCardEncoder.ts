...
export class SimulationCardEncoder {
    public encode(card: Card | null | undefined, activePlayer?: Player): SimulationCardView | undefined {
        ...
        return {
            ...
            printedCost: this.readCardDataValue<number>(card, 'cost'),
            printedHp: this.readCardDataValue<number>(card, 'hp'),
            printedPower: this.readCardDataValue<number>(card, 'power'),
            upgradePower: this.readCardDataValue<number>(card, 'upgradePower'),
            upgradeHp: this.readCardDataValue<number>(card, 'upgradeHp'),
            controllerId: card.controller?.id,
            ownerId: card.owner?.id,
            type: card.type,
            printedType: this.readStringValue(card, 'printedType'),
            types: this.toStringArray(this.readCardDataValue<string[]>(card, 'types')),
            arena: this.readCardDataValue<string>(card, 'arena'),
            aspects: this.toStringArray(this.readCardDataValue<string[]>(card, 'aspects') ?? this.readValue(card, 'aspects')),
            traits: this.toStringArray(this.readCardDataValue<string[]>(card, 'traits') ?? this.readValue(card, 'traits')),
            keywords: this.toStringArray(this.readCardDataValue<string[]>(card, 'keywords') ?? this.readValue(card, 'keywords')),
            unique: this.readCardDataValue<boolean>(card, 'unique'),
